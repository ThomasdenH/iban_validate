...
fn test_registry_examples() -> Result<(), ParseIbanError> {
    ...
    for (country_code, bank_identifier, branch_identifier, bban, iban_electronic, iban_print) in
        examples
    {
        let iban_1 = iban_electronic
            .parse::<Iban>()
            .expect("could not parse electronic format");
        let iban_2 = iban_print
            .parse::<Iban>()
            .expect("could not parse print format");
        assert_eq!(
            iban_1, iban_2,
            "parsed ibans should be identical, regardless of format"
        );
        assert_eq!(iban_1.country_code(), *country_code, "country codes do not match");
        assert_eq!(iban_1.bank_identifier(), *bank_identifier, "bank identifiers do not match");
        assert_eq!(iban_1.branch_identifier(), *branch_identifier, "branch identifiers do not match");
        assert_eq!(iban_1.bban(), *bban, "the bban doesn't match");
    }
    Ok(())
}
