...
fn test_valid_countries() {
    let valid_iban_countries = ["AD1200012030200359100100",
    ...
    for &i in valid_iban_countries.into_iter() {
        asserting("Valid country format")
            .that(&validate_iban_country(i))
            .named(&i)
            .is_equal_to(&IbanCountryResult::Valid);
    }
}

#[test]
/// This test checks whether invalid country formats are recognized as such by
/// `validate_iban_country()`.
fn test_invalid_country_format() {
    let valid_iban_counties = ["AD12BD012030200359100100",
                               "AE07ABCD234567890123456",
                               "AL47212110090000AB023569874",
                               "AT61190430234533203672",
                               "AZ21N00Z000000000137010001944",
                               "BA3912900794010284AC",
                               "BE685390075470",
                               "BG80BN96611020345678",
                               "BH67BG00001299123456",
                               "BR18003605000010009795493C1",
                               "BY13NBRB36009000002Z00AB00"];

    for &i in valid_iban_counties.into_iter() {
        asserting("Invalid country formats")
            .that(&validate_iban_country(i))
            .named(&i)
            .is_equal_to(&IbanCountryResult::Invalid);
    }
}

#[test]
/// This test checks whether an iban with an unknown country is recognized as such by
/// `validate_iban_country()`.
fn test_unknown_country() {
    let iban_unknown = "ZZ00273912631298461";

    asserting("Unknown country")
        .that(&validate_iban_country(iban_unknown))
        .named(&iban_unknown)
        .is_equal_to(IbanCountryResult::CountryUnknown);
}
