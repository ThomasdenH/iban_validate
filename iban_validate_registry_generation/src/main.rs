...
\tmatch country_code {{"
    ...
    for record in &contents.records {
        if let Some((mut start, mut end)) = record.bank_identifier_position {
            ...
            // We do this by considering the bank identifier pattern
            // (i.e. "4!n") and comparing its length to the range.
            if let Some(bank_identifier_pattern) = &record.bank_identifier_pattern {
                ...
                if end - start != bank_identifier_length {
                    writeln!(&mut writer, "\t\t// The bank identifier length ({}) does not match the range ({}..{}) in the registry. Using length as truth.", bank_identifier_length, start, end)?;
                ...
                if matches!(record.country_code, "MK" | "SE" | "ST") {
                    // For these countries the bank identifier matches neither
                    // that in the BBAN nor the IBAN.
                    assert_ne!(record.bban[start..end], bank_identifier_example);
                    assert_ne!(
                        record.iban_electronic[start + 4..end + 4],
                        bank_identifier_example
                    );

                    // For ST, weirdly the PDF BBAN does match the
                    // bank_identifier, but the txt doesn't.
                } else {
                    // We match the example bank identifier against the BBAN
                    // and against the IBAN. Unfortunately, the BBAN does not
                    // necessarily match the IBAN and the bank identifier may
                    // match either of the two. As such, we check that it
                    // matches one of the two and then we're happy.
            ...
            // No range given. There is no bank identifier and as such we do
            // not expect any of the other options to be set.
            assert!(record.bank_identifier_pattern.is_none());
            assert!(record.bank_identifier_example.is_none());
            ...
            if let Some(branch_identifier_example) = record.branch_identifier_example {
                if branch_identifier_example.len() != end - start {
                    if branch_identifier_example.len() == (end - 1) - start {
                        ...
                        writeln!(write, "\t\t// The registry branch example (\"{}\") does not have the length as expected from the position range ({}..{}).\n\t\t// Assume the example is correct, see generation code for details.", branch_identifier_example, start, end)?;
                        end -= 1;
                    } else {
                ...
                // The entry for Jordan is incorrect.
                assert_eq!(record.country_code, "JO");

                // There is no example even though there is a range. We will
                // just use the range and accept that there is no example to
                // test against. This seems to match other sources, i.e.
                // https://www.xe.com/nl/ibancalculator/jordan/

                // Note that the .PDF version of the registry is incorrect. The
                // bank position should be 1-4 but is 5-8, the branch position
                // should be 5-8 but is empty.
                assert_eq!(record.branch_identifier_position, Some((5, 8)));
                writeln!(write, "\t\t// The registry doesn't provide an example.")?;
            }
            writeln!(
                write,
                "\t\t\"{}\" => Some({}..{}),",
